...
/**
 * The upcoming-releases calendar body, with no screen chrome (no SafeAreaView,
 * ScrollView, or page header) so it can be embedded inside any scrolling parent
 * — the Library "Upcoming" tab on phones and the TV Calendar tab alike.
 */
    ...
    <View>
      ...
      {!loading && !hasAnyEvents && (
        <View style={upcomingStyles.emptyContainer}>
          ...
          <Text style={upcomingStyles.emptySubText}>
            ...
            set reminders from title detail pages when a future release date is
            available.
          </Text>
        </View>
      )}
      ...
      {!loading &&
        sectionOrder.map((section) => {
          ...
          return (
            <View key={section} style={upcomingStyles.section}>
              <Text style={upcomingStyles.sectionTitle}>{section}</Text>
              {items.map((event) => (
                <CalendarEventCard
                  key={event.id}
                  event={event}
      ...
      {activeReminderCount > 0 && (
        <View style={upcomingStyles.section}>
          <Text style={upcomingStyles.sectionTitle}>
            Active Reminders ({activeReminderCount})
          </Text>
          <Text style={upcomingStyles.reminderHint}>
            You'll be notified before these titles release. Tap the bell on any
            event to manage reminders.
          </Text>
        </View>
      )}
    </View>
